	...
	useEffect(() => {
		...
		if (role === "assistant") {
			if (content.includes("part 2")) {
				setTestPart(SpeakingTestPart.part2);
				setIsPartTwo(true);
			} else if (content.includes("part 3")) {
				setTestPart(SpeakingTestPart.part3);
				setIsPartTwo(false);
				setIsPartThree(true);
			}
		}
					...
					trigger={
						<div className="flex items-center gap-2 mb-2 max-sm:flex-col">
							...
							{!(isPartTwo && isPartThree) && (
								<span className="text-xs px-2 py-1 bg-red-300 text-yellow-900 rounded-full font-semibold uppercase max-sm:px-3 max-sm:py-2">
...
export default SpeakingVapiModel;
